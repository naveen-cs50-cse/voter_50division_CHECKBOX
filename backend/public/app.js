...
function normalizeVoter(v) {
  return {
    ...
    vote_status: (v.support_status || "UNDECIDED").toLowerCase()
...
async function loadSupportCounts() {
  try {
    const res = await fetch(`${API_BASE}/api/voters/support-counts`);

    const data = await res.json();

    document.getElementById("countOur").textContent = data.OUR || 0;
    document.getElementById("countOpp").textContent = data.OPP || 0;
    document.getElementById("countUndecided").textContent = data.UNDECIDED || 0;
  } catch (err) {
